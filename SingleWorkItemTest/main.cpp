...
int main(int argc, char *argv[])
{
    ...
    cl::Kernel singleMedianKernel5(program, "Median5SingleWorkItem");
    cl::Kernel sobelKernel(program, "SobelEdge");
    cl::Kernel singleSobelKernel(program, "SobelSingleWorkItem");
    ...
    while(1)
    {
        ...
        if(filterType == 3)
        {
            if(enSingleKernel == 1)
            {
                singleMedianKernel5.setArg(0, d_frame_in);
                singleMedianKernel5.setArg(1, d_frame_out);
                
                queue.enqueueTask(singleMedianKernel5);
                queue.finish();
            }
            else
            {
...
//            queue.finish();
            singleSobelKernel.setArg(0, d_frame_out);
            singleSobelKernel.setArg(1, d_frame_in);
            
            queue.enqueueTask(singleSobelKernel);
            queue.finish();
